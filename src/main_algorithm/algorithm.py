...
class EMRQualityInspectionAlgorithm:
    def __init__(self, entity_dict_path, ner_model_path, kg_graph_path, diagnosis_path, corrector_path, acute_illness_path, body_path) -> None:
        ...
        # 在 C:\Users\huawei\Desktop\qualitycontrol\src\main_algorithm\algorithm.py 文件中
# 假设 match_model, ner_model, spacy_model, graph_model 已经正确定义
# 同时 diagnosis_path 也已经正确获取

# 修改前的代码
# self._item_veto_predictor = ItemVeto(match_model, ner_model, spacy_model, graph_model)

# 修改后的代码，添加 diagnosis_path 参数
    ...
    def _general_project(self,emr):
        ...
        """
        return self._general_project_predictor.check(emr)
    
    def _chief_complaint(self, emr):
        """
        主诉（10分）
        ...
        return self._chief_complaint_predictor.check(emr)

    def _present_illness(self, emr):
        """
        现病史 (15分)
        ...
        """
        return self._present_illness_predictor.check(emr)
    
    def _past_illness(self, emr):
        """
        既往史及其它病史 (10分)
        7.记录重要的或与本病相关的既往史。
        ...
        """
        return self._past_illness_predictor.check(emr)
    
    def _physical_examination(self, emr):
        """
        ...
        """
        return self._physical_examination_predictor.check(emr)
    
    def _auxiliary_inspection(self, emr):
        """
        辅助检查 (10分)
        ...
        """
        return self._auxiliary_inspection_predictor.check(emr)

    def _diagnosis(self, emr):
        """
        诊断 (10分)
        ...
        """
        return self._diagnosis_predictor.check(emr)
    
    def _signature(self, emr):
        """
        签名（5分)
        ...
        """
        return self._signature_predictor.check(emr)
    ...
    def _item_veto(self, emr):
        ...
        """
        return self._item_veto_predictor.check(emr)

    def _medical_record(self, emr):
        """
        ...
        """
        return self._medical_record_predictor.check(emr)


    def emr_quality_inspection(self,emr):
        """
        Args:
            emr:{'姓名':'***', '性别':'*', '接诊时间':'*****',...}
        Return:
        ...
        reason = {}
        # 单项否决
        item_veto = self._item_veto(emr)
        # 阈值
        threshold = 100
        score += item_veto['score']
        # print(f'测试：{score} {item_veto["score"]}')
        # 病历有单项否决项
        if item_veto['vote'] != '无否决':
            if item_veto['vote'] =='乙级病历':
                # 阈值
                threshold = 90
                # 添加单项否决
                reason['vote'] = item_veto['reason']
            # 同上
            if item_veto['vote'] =='丙级病历':
                threshold = 75
                reason['vote'] = item_veto['reason']
            # ans = {
            #     'level':item_veto['vote'],
            #     'score': -1,
            #     'reason': {'item_veto':item_veto}
            # }
            # return ans
        ...
        fields = [
            ...
            '_present_illness',
            '_past_illness',
            '_physical_examination',
            '_auxiliary_inspection',
            '_diagnosis',
            '_signature',
            '_medical_record'
        ]
        # 预测模型
        disease_predict = self._disease_predict(emr)

        for field in fields:
            field_ans = getattr(self, field)(emr,)
            score += field_ans['score']
            reason[field[1:]] = field_ans
