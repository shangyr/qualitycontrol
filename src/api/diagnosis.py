...
class Diagnosis:
    ...
    def _item_4(self, emr, parm):
        ...
        score = 0
        # 不同模块之间的一致性检查
        pair_list = parm.get("Parameter0")
        match_pair = [[pair_list[0], pair_list[1]], [pair_list[2], pair_list[3]], [pair_list[3], pair_list[4]], [pair_list[1],pair_list[4]]]
        ...
        if score<0:
            return {"item": 4,"是否扣分": "是", "score": score, "reason": reason, "knowledge_graph": {}}
        else:
            return {"item": 4, "是否扣分": "否", "score": score, "reason": reason, "knowledge_graph": {}}

    def _item_25(self, emr, parm):
        reason = ''
        score = 0
        ...
        # diagnosis_pattern = r"；(.*?)$"
        # 使用正则表达式去除数字和顿号
        cleaned_text = re.sub(r"\d+、", "", emr.get(item, ""))
        # 去除分号
        ...
        if diagnosis_list:
            ...
            for diagnosis in diagnosis_list:
                ...
                if sym == 0:
                    ...
                    if len(paths) == 0:
                        score -= 5
                        reason += f"次要诊断{diagnosis}未在{'，'.join(items)}中有相关记录。\n"

        if score<0:
            return {"item": 25,"是否扣分": "是", "score": score, "reason": reason, "knowledge_graph": {}}
        else:
            return {"item": 25, "是否扣分": "否", "score": 0, "reason": "", "knowledge_graph": {}}


    def _item_51(self, emr, parm):
        """
        当前病历与既往3个月内病历记录的一致性检测。
        """
        reason = ''
        score = 0
        diagnosis = parm.get('Parameter0')
        matched_entities = self.match_model.find_entities(emr[diagnosis[0]])['pos']
        ...
        if score<0:
            return {"item": 51,"是否扣分": "是", "score": score, "reason": reason, "knowledge_graph": {}}
        else:
            return {"item": 51, "是否扣分": "否", "score": score, "reason": reason, "knowledge_graph": {}}
