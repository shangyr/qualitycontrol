...
import json
from src.extract_entity.cmed_tools.utils import load_vocab
from src.extract_entity.cmed_tools.ner_constant import *

from src.extract_entity.cmed_tools.model_ner import BERT_LSTM_CRF
import os


class CmedMedicalNer(object):
    def __init__(self, model_path='medical_ner'):
        self.NEWPATH = os.path.join(model_path,'model.pkl') 
        vocab_path = os.path.join(model_path,'vocab.txt') 
        self.vocab = load_vocab(vocab_path)
        self.vocab_reverse = {v: k for k, v in self.vocab.items()}

        self.model = BERT_LSTM_CRF(model_path, tagset_size, 768, 200, 2,
        ...
        for i, dev_batch in enumerate(test_loader):
            ...
            if entity_mark is not None:
                for item, ent in entity_mark.items():
                    ...
                    entity_list.append((entity, tag_dic[tag]))
            # print(entity_list)
        return entity_list
