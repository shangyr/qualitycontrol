...
class KBDPTTrainer(BaseTrainer):
    ...
    def _fit_batch(self,data):
        ...
        logits, logits_prompt = self.model(data['input_ids'],data['attention_mask'], data['mask_idx'])
        loss = self.loss_func(logits,data['label'].float())
        loss_prompt = self.loss_func(logits_prompt, data['label_prompt'].float())
        return loss + loss_prompt
    ...
    def _inference_batch(self,data):
        ...
        for i in range(0,batch_size,self.config.batch_size * 10):
            input_ids = data['input_ids'][i:i+self.config.batch_size * 10]
            attention_mask = data['attention_mask'][i:i+self.config.batch_size * 10]
            mask_idx = data['mask_idx'][i:i+self.config.batch_size * 10]
            _logits, _logits_prompt = self.model(input_ids,attention_mask, mask_idx) # 
            logits.append(_logits)
            logits_prompt.append(_logits_prompt)
        logits = torch.cat(logits, dim=0)
        logits_prompt = torch.cat(logits_prompt, dim=0)
        logits_prompt = torch.softmax(logits_prompt[:,1], dim=0).unsqueeze(1) # [path_num, 2]
        logits = (logits * logits_prompt).sum(dim=0,keepdim=True) # 软投票
        return logits # [1, num_class]
    ...
    def _to_gpu(self,data):
        ...
        data['mask_idx'] = data['mask_idx'].to(torch.cuda.current_device())
        data['label'] = data['label'].to(torch.cuda.current_device())
        if 'label_prompt' in data:
            data['label_prompt'] = data['label_prompt'].to(torch.cuda.current_device())
        return data
